...
export default function TallesPage() {
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      ...
      {/* HERO */}
      <section className="max-w-7xl mx-auto px-4 py-12 lg:py-14 text-center">
        <p className="text-[#D4FF00] text-xs font-black uppercase tracking-[0.2em] mb-3">REFERENCIA</p>
      ...
      <section className="border-t border-white/[0.06] py-14 bg-[#050505]">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {PALETAS.map((p, i) => (
              <div key={i} className={`bg-[#121212] border rounded-2xl p-6 ${i === 1 ? 'border-[#D4FF00]/30' : 'border-white/[0.08]'}`}>
                {i === 1 && (
                  <span className="text-[9px] font-black uppercase tracking-widest text-[#D4FF00] bg-[#D4FF00]/10 px-2 py-1 rounded mb-3 inline-block">
                    MÁS VENDIDO
                  </span>
                )}
                <h3 className={`font-black text-lg mb-3 ${i === 1 ? 'text-[#D4FF00]' : 'text-white'}`}>{p.nivel}</h3>
                ...
                <p className="mt-4 text-[10px] text-[#D4FF00] font-semibold uppercase border-t border-white/[0.06] pt-3">
                  {p.recomendado}
                </p>
              </div>
            ))}
      ...
      <section className="border-t border-white/[0.06] py-14">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="bg-[#121212] border border-white/[0.08] rounded-2xl overflow-hidden overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/[0.08] bg-[#0d0d0d]">
                  <th className="text-left text-xs font-black uppercase tracking-widest text-[#D4FF00] px-5 py-3">ARG</th>
        ...
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="bg-[#121212] border border-white/[0.08] rounded-2xl overflow-hidden overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/[0.08] bg-[#0d0d0d]">
                  <th className="text-left text-xs font-black uppercase tracking-widest text-[#D4FF00] px-5 py-3">Talle</th>
